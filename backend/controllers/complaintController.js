  ...
  try {
    ...
    if (!room) {
      return res.status(400).json({
        message: 'You must be allocated a room before filing a complaint',
      });
    }
...
const myComplaints = async (req, res, next) => {
  try {
    ...
    const escalated = await Promise.all(
      complaints.map(applyEscalationIfDue)
    );
...
// @route GET /api/complaints (admin/warden)
...
// Auto escalate complaint priority
async function applyEscalationIfDue(complaint) {
  ...
  const escalationHours =
    Number(process.env.COMPLAINT_ESCALATION_HOURS) || 48;

  const hoursSinceEscalation =
    (Date.now() -
      new Date(complaint.lastEscalatedAt).getTime()) /
    (1000 * 60 * 60);
...
const rateComplaint = async (req, res, next) => {
  try {
    ...
    if (complaint.status !== 'resolved') {
      return res.status(400).json({
        message: 'You can only rate a resolved complaint',
      });
    }
